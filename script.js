          ...
          dataProductos.forEach((productoEnArray, indice)=> {
              ...
              divProductos.innerHTML += `
               <div class="card border-success mb-3" id="producto${indice}" style="max-width: 20rem; margin:8px">
                   <div class="card-header">${productoEnArray.clasificacion}</div>
          ...
          dataProductos.forEach((productoEnArray, indice) => {
              document.getElementById(`boton${indice}`).addEventListener('click', () => {
                   if(productos.find(producto => producto.nombre == productoEnArray.nombre)) {
                       ...
                       let nuevoProducto = new Producto(productoEnArray.clasificacion, productoEnArray.nombre,  
                       productoEnArray.talle, productoEnArray.precio, productoEnArray.stock, productoEnArray.img)
